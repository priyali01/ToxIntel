...
ToxNet---a shared-backbone multi-task neural network utilizing a Two-Pass
training strategy and Per-Endpoint Masked Focal Loss.
We further introduce Geometric Refinement using Tomek Links to sharpen the
decision boundary in the chemical embedding space.
On the scaffold-stratified Tox21 benchmark, ToxNet achieves a macro AUPRC of
\textbf{0.366} and a macro AUROC of 0.844---substantially outperforming 
logistic regression (AUPRC 0.240) and optimized XGBoost (AUPRC 0.310) baselines.
...
To address this, we apply \textit{Geometric Refinement} using Tomek Links
\cite{tomek1976} to remove ambiguous majority samples in the embedding space.

\subsection{Two-Pass ToxNet Architecture and Focal Loss}
ToxNet is trained via a Two-Pass strategy. In Pass 1, a \textit{ToxNetLite}
embedding expert learns to compress high-dimensional fingerprints into a
256-dimensional chemical manifold.
In Pass 2, task-specific heads are fine-tuned on these embeddings while the
backbone is frozen.
The shared backbone consists of four fully connected layers...
...
ToxNet achieves a macro AUPRC of 0.366 and macro AUROC of 0.844, representing
a 4.5$\times$ improvement over the random baseline and a 18.1\% relative
